$(document).ready(function () { //загружаем все скрипты только после того, как документ html будет полностью готов
	...
	function validateForms(form) {
		$(form).validate({//берем блок с id="consultation" и внутри него находим тег form
			rules: {
				name: "required", //сделали поля обязательными для заполнения (выполняется по аотрибуту name). В html теперь не надо прописывать required для инпутов
				phone: "required",
				email: { //для email прописываем составное правило
					required: true,
					email: true //проверяем, действительно ли пользователь ввел email
				}
			},
			messages: {
	...
	//задаем значения для переменной form
	validateForms('#consultation-form'); //для каждй используемой формы надо задать свой id
